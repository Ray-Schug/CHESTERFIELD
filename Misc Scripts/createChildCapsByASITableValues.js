...
function createChildCapsByASITableValues() {
	...
	if (relationshipBizDomain == null || relationshipBizDomain.getDescription() == null || relationshipBizDomain.getDescription().length() <= 0) {
		return;
	}
...
function getNewRelationshipIDs(asitModel, columnNameList) {
	...
	if (asitColumnNames != null && asitColumnNames.size() >= columnNameList.size() && asitColumnNames.contains(columnNameList.get(0)) && asitColumnNames.contains(columnNameList.get(1))) {
		...
		for (var ii = 0; ii < rowIndexArray.size(); ii++) {
			...
			for (var j = 0; j < fields.size(); j++) {
				if (rowIndexValue.equals(fields.get(j).getRowIndex())) {
					...
					if (columnNameList.get(0).equals(label) && (value == null || value.equals(""))) {
						break;
					}
			...
			if (recordTypeValues == null || recordTypeValues.length != 4 || recordTypeValues[0] == null || recordTypeValues[0] == "" || recordTypeValues[1] == null || recordTypeValues[1] == "" || recordTypeValues[2] == null || recordTypeValues[2] == "" || recordTypeValues[3] == null || recordTypeValues[3] == "") {
				continue;
			}
...
function getExistingChildsMap(parentRecordID, associatedFormsFlag) {
	...
	for (var i = 0; i < capScriptModels.length; i++) {
		...
		if (capScriptModel != null) {
			...
			if (capScriptModel.getCapID() != null && project != null && project.getProject() != null && associatedFormsFlag.equals(project.getProject().getRelationShip())) {
...
function getRecordTypeListFromASITableFields(fields, columnNameList, existingChildRecordsMap) {
	...
	for (var ii = 0; ii < rowIndexArray.size(); ii++) {
		...
		if (currentRelationshipIDValue != null && currentRelationshipIDValue != "" && existingChildRecordsMap != null && existingChildRecordsMap.size() > 0 && existingChildRecordsMap.containsKey(currentRecordTypeValue + "||" + currentRelationshipIDValue)) {
			continue;
		}
