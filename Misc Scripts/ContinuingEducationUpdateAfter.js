...
// 1. get continuing education passResult 
var passResult = aa.continuingEducation.isContinuingEducationPassed(capScriptModel);
...
if (passResult.getSuccess()) {
	...
	if (passResult.getOutput().booleanValue()) {
		...
		//continuing education is passed
		{
	...
	} //if continuing education is passed
} //if get isPassed successfully
else {
	aa.print("ERROR: Failed to get isContinuingEducationPassed(" + capId + "): " + passResult.getErrorMessage());
	aa.env.setValue("ScriptReturnCode", "0");
	aa.env.setValue("ScriptReturnMessage", "ERROR: Failed to get isContinuingEducationPassed(" + capId + "): " + passResult.getErrorMessage());
